...
void configure_reconstruction(struct recon_metadata *mr){
    ...
    int n_proj_pull=rp.n_readings;
    
    // copy this info into our recon metadata
    mr->ri.n_ffs=n_ffs;
    mr->ri.n_slices_requested=-1;
    mr->ri.n_slices_recon=-1;
    mr->ri.n_slices_block=-1;
    mr->ri.n_blocks=-1;
    mr->ri.idx_slice_start=-1;
    mr->ri.idx_slice_end=-1; 
    mr->ri.recon_start_pos=-1;
    mr->ri.recon_end_pos=-1;
    mr->ri.idx_pull_start=0;
    mr->ri.idx_pull_end=n_proj_pull-1;
    ...
    mr->ctd.image=(float*)calloc(1,sizeof(float));
}
